...
can be the forward model. Its **flagship partner is
[APSIM](https://www.apsim.info)**, the agricultural-systems simulator,
coupled in-process through the
[`apsimx`](https://cran.r-project.org/package=apsimx) package – the use
case PESTO’s callback was built for. The same callback couples any other
process-based simulator just as directly: the groundwater / hydrology
models of PEST’s own heritage, other crop models, and compartmental ODE
systems.
...
That benchmark’s frozen table also carries a CI90 of about 0.13 against
a nominal 0.90 for PESTO’s raw ensemble intervals. That figure predates
the current development version: the smoother assimilated the same
unperturbed observation vector into every realisation, which is now
fixed by per-realisation observation perturbation under an ES-MDA
schedule, graded against the closed-form conjugate posterior in both
moments. Re-running the benchmark needs the external PEST and PEST++
binaries, so its calibration row still shows the old behaviour.
...
to that simulator’s outputs. PESTO’s flagship partner is **APSIM**:
[`apsim_callback()`](https://max578.github.io/PESTO/reference/apsim_callback.md)
turns an `.apsimx` model file plus a parameter map into exactly the
forward-model callable the IES driver expects, running the ensemble
in-process via `apsimx`.
...
The full worked example is in the *Calibrating APSIM with PESTO*
vignette (it needs APSIM installed, so its run blocks are disabled on
CRAN). The same
[`pesto_ies_callback()`](https://max578.github.io/PESTO/reference/pesto_ies_callback.md)
pattern couples any other simulator: a CRAN-safe, fully-runnable
agricultural demonstration using the built-in
...
- [`vignette("apsim-case-study", package = "PESTO")`](https://max578.github.io/PESTO/articles/apsim-case-study.md)
  – Publication-grade APSIM wheat calibration: synthetic-truth
  recovery + real-data calibration with uncertainty
