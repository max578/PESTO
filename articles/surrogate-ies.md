...
[`surrogate_ensemble_update()`](https://max578.github.io/PESTO/reference/surrogate_ensemble_update.md)
performs the classification and blended update:
...
4.  Apply the mean-offset bias correction.
5.  Compute the IES update on the blended ensemble.

**What it does not do (in this release):** it does not itself call a
forward model for the “full model” realisations – `obs_ensemble` above
is the caller’s already-evaluated ensemble, and step 3’s classification
only chooses which rows of that pre-computed ensemble to keep as-is
versus replace with the surrogate prediction. `n_model_runs` and
`savings_pct` below therefore report how many realisations *could* skip
a forward-model call, not evaluations actually avoided in this run – the
caller has already paid for every one of them. Realising the saving
requires wiring a `forward_model` argument that this verb does not yet
accept; treat the numbers as a projection of the achievable saving, not
a report of compute already saved. The “mean-offset bias correction” is,
as named, a single additive offset between the model- and
surrogate-evaluated rows – not the variance-reduction control-variate
estimator of Glasserman (2003) cited below, which
...
realisations back toward the prior the way the standard IES update does.
...
cat("Realisations classified 'needs full model':", result$n_model_runs, "\n")
#> Realisations classified 'needs full model': 0
cat("Realisations classified 'surrogate suffices':", result$n_surrogate_runs, "\n")
#> Realisations classified 'surrogate suffices': 50
cat("Total realisations:", result$n_total, "\n")
#> Total realisations: 50
cat("Projected saving if wired to skip forward-model calls:",
    sprintf("%.0f%%", result$savings_pct), "\n")
#> Projected saving if wired to skip forward-model calls: 100%
...
surrogate-safe, 1 - n\_{\text{model}}/n\_{\text{total}} – a
*projection*, per the caveat in Step 4, since this release does not yet
skip the forward-model call it counts as saved. Because the GP is
trained on the current ensemble and predicts at those same points, in a
smooth regime it is confident enough to classify essentially the whole
ensemble as surrogate-safe at the default threshold – so the projected
saving is high *by construction*. What decides whether wiring up the
real saving would be worth it is therefore not how many realisations get
classified as surrogate-safe but whether the surrogate’s answer stays
accurate, which the *Effect of nonlinearity* section measures next.
