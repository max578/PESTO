# PESTO ensemble-manifest contract (v1.0.0)
...
conformance_test: "verify_manifest() integrity check + write/read round-trip"
...
- Treat the manifest properties as a **public, versioned contract**. Bump
  `schema_version` (currently `1.0.0`) on any breaking change; consumers branch
  on it.
...
closes (e.g. `kernR::mmd_ppc()` traces back to the producing run).
