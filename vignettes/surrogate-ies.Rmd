...
`surrogate_ensemble_update()` performs the full algorithm:
...
The saving is the fraction of realisations the surrogate handles,
$1 - n_{\text{model}}/n_{\text{total}}$. Because the GP is trained on the current
ensemble and predicts at those same points, in a smooth regime it is confident
enough to handle essentially the whole ensemble at the default threshold -- so
the saving is high *by construction*. What decides whether the surrogate is
worth using is therefore not how many realisations it handles but whether its
answer stays accurate, which the *Effect of nonlinearity* section measures next.
