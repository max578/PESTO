...
introduces a versioned S7 contract object —
`pesto_ensemble_manifest` — plus YAML+CSV persistence with SHA-256
integrity checking.
...
weaker contract; `verify_manifest()` returns `ok = NA` rather than a
spurious `FALSE` because CSV formatter precision loss (~1 ULP at
IEEE 754 epsilon) is enough to flip the hash:
