...
**PESTO** (*Parameter ESTimation Optimised* -- the PEST approach brought to R)
is an R package for high-performance parameter estimation,
...
the forward model. Its flagship partner is the **APSIM** agricultural-systems
simulator, coupled in-process through `apsim_callback()` and the `apsimx`
package -- see the *Calibrating APSIM with PESTO* vignette -- while the same
callback couples hydrological models, other crop models, and ODE systems
equally.
...
- **APSIM coupling**: `apsim_callback()` drives `apsimx` ensembles in-process
  -- PESTO's flagship simulator partner
...
`ensemble_solution_adaptive()` wraps the IES kernel with automatic SVD backend
selection -- a randomised SVD for low-rank problems, otherwise a dense LAPACK /
Accelerate decomposition -- and returns timing diagnostics. It is the convenient
entry point when the best backend for a problem size is not known in advance.
