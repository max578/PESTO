...
`PESTO` follows a published two-phase policy. The phase boundary is
the 1.0.0 release.
...
- `0.2.0` — added `pesto_ies_callback()` (in-process IES driver) and
  `apsim_callback()` (apsimx adapter). **No breaking changes.**
- `0.3.0` — added `pesto_ensemble_manifest` (S7 cross-package
  contract) plus `as_manifest()`, `write_manifest()`,
  `read_manifest()`, `verify_manifest()`. **No breaking changes.**
- `0.3.1` — added `format = c("rds","both","csv")` to
  `write_manifest()`; backwards-compatible YAML.
- `0.3.2` — renamed `format = "csv"` to `"csv_unverified"` at every
  call site; legacy `"csv"` still accepted with a deprecation
  warning, so 0.3.1 manifests round-trip cleanly.
- `0.3.3` — Makevars FLIBS portability fix; no API change.

The policy is "additive-by-intent" rather than "frozen": pre-1.0
reserves the right to break an existing signature when a design
flaw surfaces, but every such change must be:
...
3. Where feasible, accompanied by a temporary back-compat shim
   (the v0.3.2 `csv` → `csv_unverified` rename is the template:
   legacy spelling still works with a deprecation warning).
...
— the same policy as `glmnet`, `mgcv`, and other long-lived
solver-style packages:
...
This policy is chosen because `PESTO` is invoked as the IES /
surrogate backend from pipeline code that the maintainer cannot
edit — kernR consumes the `pesto_ensemble_manifest` S7 contract;
APSIM ensemble loops via `pesto_ies_callback()` are written once
and run for the life of a calibration campaign. Silent breakage
...
<https://github.com/AAGI-AUS/PESTO/issues> with the version you
