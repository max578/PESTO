...
`apsim_callback()` turns an `.apsimx` model file plus a parameter map
into exactly the forward-model callable the IES driver expects, running
the ensemble in-process via `apsimx`.
...
CRAN). The same `pesto_ies_callback()` pattern couples any other
simulator: a CRAN-safe, fully-runnable agricultural demonstration using
the built-in `crop_growth_forward_model()` is in that vignette, and the
...
- `vignette("getting-started", package = "PESTO")` – Introduction and
  basic usage
- `vignette("apsim-callback", package = "PESTO")` – Coupling APSIM (and
  other simulators) via the in-process callback
- `vignette("apsim-case-study", package = "PESTO")` – Publication-grade
  APSIM wheat calibration: synthetic-truth recovery + real-data
